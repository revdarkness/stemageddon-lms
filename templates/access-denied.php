...
?>
<section class="sglms-access-denied" style="max-width:640px;margin:40px auto;padding:24px;border:1px solid #dcdcde;border-radius:8px;background:#fff;text-align:center;">
	<h2 style="margin-top:0;"><?php esc_html_e( 'This content is restricted', 'sglms' ); ?></h2>
	...
	<p style="margin-top:20px;">
		<?php if ( ! $sglms_logged_in ) : ?>
			<a class="button button-primary" href="<?php echo esc_url( wp_login_url( $sglms_redirect ) ); ?>">
				<?php esc_html_e( 'Log in', 'sglms' ); ?>
			</a>
		<?php endif; ?>

		<?php if ( $sglms_course instanceof WP_Post ) : ?>
			<a class="button" href="<?php echo esc_url( get_permalink( $sglms_course ) ); ?>">
				<?php esc_html_e( 'View course & enroll', 'sglms' ); ?>
			</a>
		<?php else : ?>
			<a class="button" href="<?php echo esc_url( Sglms_Templates::page_url( 'sglms_page_catalog' ) ); ?>">
